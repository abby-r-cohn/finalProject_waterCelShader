...
void Plane::collide(PointMass &pm) {
  // TODO (Part 3): Handle collisions with planes.
    double direction = dot(pm.position - point, normal);
    double last_direction = dot(pm.last_position - point, normal);
    if (direction < 0 && last_direction > 0) {
        Vector3D tangent_point = pm.position + normal.unit() * dot(point - pm.position, normal);
